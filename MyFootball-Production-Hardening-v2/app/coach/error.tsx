...
export default function CoachError({
  ...
  useEffect(() => {
    console.error("Coach Portal Error:", error);
  }, [error]);
    ...
    <div className="min-h-screen bg-background text-foreground flex items-center justify-center p-4 font-sans">
      <div className="max-w-md w-full rounded-3xl bg-card border border-border p-6 sm:p-8 shadow-2xl space-y-6 text-center">
        ...
        <div className="space-y-2">
          <h2 className="text-xl font-extrabold tracking-tight text-foreground">Coach Portal Error</h2>
          <p className="text-sm text-muted-foreground">
            Could not synchronize club rosters and tactical sheets. Please ensure you are signed in with the correct coaching credentials.
          </p>
          {error?.message && (
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 pt-2">
          ...
          >
            <RotateCcw size={16} /> Retry Portal
          </button>
          <Link
            href="/"

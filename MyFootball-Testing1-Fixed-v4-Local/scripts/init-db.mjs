...
for (const file of files) {
  ...
  const migrationSql = fs
    ...
    .filter((migration) => /^\d{4}_.+\.sql$/.test(migration))
    .sort()
    .map((migration) => fs.readFileSync(path.resolve("drizzle", migration), "utf8"))
  ...
  const alterStatements = [
    ...
    "ALTER TABLE divisions ADD COLUMN include_third_place integer DEFAULT 0 NOT NULL",
    "ALTER TABLE divisions ADD COLUMN knockout_bye_entry_ids text DEFAULT '[]' NOT NULL",
    ...
    "ALTER TABLE fixtures ADD COLUMN referee_notes text DEFAULT '' NOT NULL",
    "ALTER TABLE fixtures ADD COLUMN winner_entry_id text",
    "ALTER TABLE fixtures ADD COLUMN loser_entry_id text",

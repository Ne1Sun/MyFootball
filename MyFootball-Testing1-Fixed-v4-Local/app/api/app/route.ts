...
export async function POST(request: Request) {
  try {
    ...
    if (payload.action === "advanceBracketWinner") {
      ...
      const currentFix = await ownedFixture(fixtureId, user.email);
    ...
    if (payload.action === "updateLiveMatch") {
      ...
      const homeScorePenalties = payload.homeScorePenalties !== undefined
        ? numberValue(payload.homeScorePenalties)
        : undefined;
      const awayScorePenalties = payload.awayScorePenalties !== undefined
        ? numberValue(payload.awayScorePenalties)
        : undefined;
      ...
      if (homeScorePenalties !== undefined) updateData.homeScorePenalties = Math.max(0, homeScorePenalties);
      if (awayScorePenalties !== undefined) updateData.awayScorePenalties = Math.max(0, awayScorePenalties);
    ...
    if (payload.action === "deleteMatchEvent") {
      ...
      const fixture = await ownedFixture(ev.fixtureId, user.email);
    ...
    if (payload.action === "updateScore") {
      ...
      const requestedStatus = clean(payload.status, 20) || owned.fixture.status;
      if (!fixtureStatuses.has(requestedStatus)) {
        return Response.json({ error: "Invalid match status." }, { status: 400 });
      }
      await db
        .update(fixtures)
        .set({

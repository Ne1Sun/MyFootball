...
export function RefereeConsoleClient({ initialData }: { initialData: RefereeData }) {
  ...
  useEffect(() => {
    ...
    if (wal && wal.fixtureId === currentFixture.id) {
      ...
      if (hasNewerClock || hasNewerScore || hasShootoutKicks || hasExtraEvents) {
        queueMicrotask(() => {
          if (cancelled) return;
          if (hasShootoutKicks) setShootoutKicks(wal.shootoutKicks);
          ...
          setWalRestoredNotice(`⚡ Recovery restored: ${wal.matchClockMinute}', ${wal.period.replaceAll("_", " ").toUpperCase()}`);
  ...
  const handleScoreAdjust = async (isHome: boolean, delta: number) => {
    ...
    setData((current) => ({
      ...current,
      fixtures: current.fixtures.map((fixture) => fixture.id === currentFixture.id
        ? { ...fixture, homeScore: newHome, awayScore: newAway }
        : fixture),
    }));
  ...
  const handleShootoutKick = async (isHome: boolean, scored: boolean, shooterName?: string) => {
    ...
    // Persist every kick so public scoreboards and recovery state agree.
    try {
    ...
    // Only advance when mathematical elimination or a completed sudden-death pair decides the winner.
    if (nextShootoutState.isFinished && nextShootoutState.winner) {

...
import { clubs, entries, fixtures, teams, tournaments, users } from "../db/schema";
...
export default async function Home() {
  ...
  // Load real public activity. Never present seeded or invented competition data.
  const publicTournamentRows = await db.select().from(tournaments).orderBy(desc(tournaments.startDate));
  const tournamentRows = publicTournamentRows.slice(0, 4);
  const fixtureRows = await db.select().from(fixtures).where(eq(fixtures.status, "in_progress")).limit(4);
  const liveEntryIds = [...new Set(fixtureRows.flatMap((fixture) => [fixture.homeEntryId, fixture.awayEntryId]))];
  const liveEntries = liveEntryIds.length
    ? await db
        .select({ entryId: entries.id, teamName: teams.name })
        .from(entries)
        .innerJoin(teams, eq(entries.teamId, teams.id))
        .where(inArray(entries.id, liveEntryIds))
    : [];
  const liveTeamNames = new Map(liveEntries.map((entry) => [entry.entryId, entry.teamName]));
  const allClubs = await db.select().from(clubs).limit(8);
  const clubCountRows = await db.select({ id: clubs.id }).from(clubs);
  const activeStates = new Set(publicTournamentRows.map((tournament) => tournament.state).filter(Boolean));
  const openRegistrations = publicTournamentRows.filter(
    (tournament) => publicTournamentStatus(tournament.status, tournament.registrationClosesAt) === "registration_open",
  ).length;

  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col selection:bg-amber-500 selection:text-slate-950 transition-colors">
      {/* Real match activity ticker */}
      <div className="bg-gradient-to-r from-amber-600 via-emerald-600 to-slate-900 text-white px-4 py-2 text-xs font-bold overflow-x-auto whitespace-nowrap flex items-center justify-between gap-6 border-b border-amber-500/30 shadow-md">
        <div className="flex items-center gap-6">
          ...
          {fixtureRows.length ? fixtureRows.map((fixture) => (
            <span key={fixture.id}>
              ⚽ <strong>{liveTeamNames.get(fixture.homeEntryId) || "Home"} {fixture.homeScore} - {fixture.awayScore} {liveTeamNames.get(fixture.awayEntryId) || "Away"}</strong> ({fixture.matchClockMinute}&apos;)
            </span>
          )) : (
            <span>No matches are live right now. Open Discover for upcoming fixtures.</span>
          )}
        </div>
      ...
      <section className="relative overflow-hidden pt-12 pb-20 px-4 lg:px-8 max-w-7xl mx-auto w-full space-y-12">
        ...
        <div className="cascade-1 text-center space-y-6 max-w-3xl mx-auto relative z-10">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-card border border-border text-amber-600 dark:text-amber-400 text-xs font-black uppercase tracking-wider shadow-inner">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 pt-2">
            ...
            <Link
              href="/organize"
              className="interactive-button px-6 py-3.5 rounded-2xl bg-card hover:bg-muted text-foreground border border-border font-extrabold text-sm transition flex items-center gap-2"
            >
        ...
        <div className="cascade-2 grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-4xl mx-auto pt-6 border-t border-border">
          <div className="interactive-card p-4 rounded-2xl bg-card border border-border text-center space-y-1">
            <strong className="font-mono font-black text-2xl sm:text-3xl text-amber-400">{activeStates.size}</strong>
            <span className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider">Active States</span>
          </div>
          <div className="interactive-card p-4 rounded-2xl bg-card border border-border text-center space-y-1">
            <strong className="font-mono font-black text-2xl sm:text-3xl text-emerald-400">{clubCountRows.length}</strong>
            <span className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider">Academies & Clubs</span>
          </div>
          <div className="interactive-card p-4 rounded-2xl bg-card border border-border text-center space-y-1">
            <strong className="font-mono font-black text-2xl sm:text-3xl text-blue-400">{fixtureRows.length}</strong>
            <span className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider">Live Matches</span>
          </div>
          <div className="interactive-card p-4 rounded-2xl bg-card border border-border text-center space-y-1">
            <strong className="font-mono font-black text-2xl sm:text-3xl text-purple-400">{openRegistrations}</strong>
            <span className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider">Registrations Open</span>
          </div>
        </div>
        ...
        <div className="cascade-3 space-y-4 pt-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            ...
            <Link
              href="/organize"
              className="interactive-card p-6 rounded-3xl bg-card border border-border hover:border-amber-500/50 transition group space-y-4 shadow-lg"
            >
              ...
              <div>
                <h3 className="text-lg font-black text-foreground group-hover:text-amber-500 transition">Tournament Director</h3>
            ...
            <Link
              href="/coach"
              className="interactive-card p-6 rounded-3xl bg-card border border-border hover:border-emerald-500/50 transition group space-y-4 shadow-lg"
            >
              ...
              <div>
                <h3 className="text-lg font-black text-foreground group-hover:text-emerald-500 transition">Coach & Academy Hub</h3>
                <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
            ...
            <Link
              href="/referee"
              className="interactive-card p-6 rounded-3xl bg-card border border-border hover:border-blue-500/50 transition group space-y-4 shadow-lg"
            >
              ...
              <div>
                <h3 className="text-lg font-black text-foreground group-hover:text-blue-500 transition">Pitch Referee Mode</h3>
                <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
            ...
            <Link
              href="/discover"
              className="interactive-card p-6 rounded-3xl bg-card border border-border hover:border-purple-500/50 transition group space-y-4 shadow-lg"
            >
              ...
              <div>
                <h3 className="text-lg font-black text-foreground group-hover:text-purple-500 transition">Fan & Scout Hub</h3>
                <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
          ...
          <div className="cascade-4 space-y-4 pt-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {tournamentRows.map((t) => (
                <div
                  key={t.id}
                  className="interactive-card p-6 rounded-3xl bg-card border border-border hover:border-amber-500/40 transition space-y-4 shadow-xl"
                >
                  <div className="flex items-center justify-between">
                    <span className="px-3 py-1 rounded-full bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 text-xs font-bold uppercase">
                      {publicTournamentStatus(t.status, t.registrationClosesAt).replaceAll("_", " ")}
                    </span>
                  ...
                  <div className="pt-2 flex items-center justify-between border-t border-border">
                    ...
                    {publicTournamentStatus(t.status, t.registrationClosesAt) === "registration_open" && (
                      <Link
                        href={`/register/${t.id}`}
        ...
        {allClubs.length > 0 && (
          <div className="p-8 rounded-3xl bg-card border border-border text-center space-y-4">
            ...
            <div className="flex flex-wrap items-center justify-center gap-6 sm:gap-10 text-foreground font-bold text-sm">
              {allClubs.map((club) => <span key={club.id}>{club.name}</span>)}
            </div>
          </div>
        )}
      </section>

...
export default function RegisterPage({
  ...
  const [user, setUser] = useState<ChatGPTUser | null>(null);
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : "Could not submit registration. Check your connection and try again.");
    } finally {
      setBusy(false);
    }
  };
  ...
  if (tournament && tournament.status !== "registration_open") {
    return (
      <main className="min-h-screen bg-background text-foreground flex items-center justify-center p-4">
        <div className="max-w-md p-8 rounded-3xl bg-card border border-border text-center space-y-4 shadow-2xl">
          <ShieldCheck size={42} className="text-amber-500 mx-auto" />
          <h1 className="text-xl font-black">Registration Closed</h1>
          <p className="text-sm text-muted-foreground">
            Team registration for <strong className="text-foreground">{tournament.name}</strong> is no longer accepting entries.
          </p>
          <Link href={`/tournament/${tournament.id}`} className="button primary inline-flex items-center gap-2 text-xs">
            View tournament
          </Link>
        </div>
      </main>
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col font-sans selection:bg-amber-500 selection:text-slate-950">
      {/* Unified App Header */}
      <AppHeader activeRoute="register" user={user} />
          ...
          <form onSubmit={submit} className="p-6 sm:p-8 rounded-3xl bg-card border border-border space-y-6 shadow-xl">
            ...
            <div className="pt-2">
              {user ? (
                <button
                  type="submit"
                  disabled={busy || !selectedDivisionId}
                ...
                >
                  {busy ? <LoaderCircle className="spin" size={18} /> : (
                    <><span>Submit Team Entry ({formatRupees(selectedDivision?.feePaise || 0)})</span><ArrowRight size={17} /></>
                  )}
                </button>
              ) : (
                <a
                  href={`/signin-with-chatgpt?return_to=${encodeURIComponent(`/register/${id}`)}`}
                  className="interactive-button w-full py-3.5 rounded-2xl bg-primary text-primary-foreground font-black text-sm shadow-xl flex items-center justify-center gap-2"
                >
                  Sign in as a fan to register <ArrowRight size={17} />
                </a>
              )}
            </div>

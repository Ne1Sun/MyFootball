    ...
    return Response.json({
      tournaments: visible.map((tournament) => {
        ...
        return {
          ...
          registeredTeams: tournamentEntries.filter((item) =>
      ...
      areas: {
        states: [
          ...new Set(
            visible.filter((item) => item.state).map((item) => item.state),
          ),
        ].sort(),
        cities: [
          ...new Set(visible.filter((item) => item.city).map((item) => item.city)),
        ].sort(),
      },
    });
  } catch (error) {

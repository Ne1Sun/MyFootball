...
type RefereeData = {
  ...
  events: MatchEvent[];
  shootoutKicks: PersistedShootoutKick[];
  user: { email: string; displayName: string };
};

type PersistedShootoutKick = {
  id: string;
  fixtureId: string;
  sequence: number;
  entryId: string;
...
const errorMessage = (error: unknown) =>
  error instanceof Error ? error.message : "Unable to save this match update.";

const postAppAction = async (payload: Record<string, unknown>) => {
  ...
  if (!response.ok) throw new Error(result.error || "Unable to save this match update.");
  return result;
};
...
const localShootoutKicks = (kicks: PersistedShootoutKick[], fixture?: Fixture) => {
  if (!fixture) return [];
  return kicks
    .filter((kick) => kick.fixtureId === fixture.id)
    .sort((left, right) => left.sequence - right.sequence)
    .map((kick) => ({
      round: Math.ceil(kick.sequence / 2),
      isHome: kick.entryId === fixture.homeEntryId,
      scored: kick.outcome === "goal",
      shooterName: kick.playerName || undefined,
    }));
};
...
export function RefereeConsoleClient({ initialData }: { initialData: RefereeData }) {
  ...
  const [selectedFixtureId, setSelectedFixtureId] = useState<string>(
    data.fixtures.find((f) => f.status === "in_progress")?.id || data.fixtures.find((f) => f.status === "scheduled")?.id || data.fixtures[0]?.id || ""
  );
  ...
  const [displayMinute, setDisplayMinute] = useState(currentFixture?.matchClockMinute || 0);
  ...
  const canRecordEvents =
    ...
    ["first_half", "second_half", "extra_time"].includes(currentFixture?.period || "");

  useEffect(() => {
    if (!currentFixture) return;
    const update = () => {
      const startedAt = currentFixture.clockStartedAt
        ? new Date(currentFixture.clockStartedAt).getTime()
        : Number.NaN;
      const elapsed = currentFixture.clockRunning && Number.isFinite(startedAt)
        ? Math.max(0, Math.floor((Date.now() - startedAt) / 60_000))
        : 0;
      setDisplayMinute(Math.min(200, (currentFixture.matchClockMinute || 0) + elapsed));
    };
    update();
    const timer = window.setInterval(update, 1_000);
    return () => window.clearInterval(timer);
  }, [currentFixture?.id, currentFixture?.matchClockMinute, currentFixture?.clockStartedAt, currentFixture?.clockRunning]);
  ...
  const refreshData = async () => {
    try {
      ...
      if (!res.ok) throw new Error(json.error || "Unable to refresh match data.");
      const refreshedFixture = (json.fixtures || []).find((fixture: Fixture) => fixture.id === selectedFixtureId) || json.fixtures?.[0];
      setShootoutKicks(localShootoutKicks(json.shootoutKicks || [], refreshedFixture));
      setData((prev) => ({
        ...prev,
        ...
        shootoutKicks: json.shootoutKicks || [],
      }));
      return true;
    } catch (error) {
  ...
  const handleUpdateClock = async (minute: number, period: string, status?: string) => {
    ...
    setData((current) => ({
      ...current,
      fixtures: current.fixtures.map((fixture) => fixture.id === currentFixture.id
        ? { ...fixture, matchClockMinute: newMinute, period: newPeriod, status: newStatus }
        : fixture),
    }));
  ...
  const handleShootoutKick = async (isHome: boolean, scored: boolean, shooterName?: string) => {
    ...
    try {
      await postAppAction({
        ...
        fixtureId: currentFixture.id,
        entryId: isHome ? currentFixture.homeEntryId : currentFixture.awayEntryId,
        playerName: shooterName || shootoutShooterName || "",
        outcome: scored ? "goal" : "miss",
      });
    } catch (error) {
      setToast(errorMessage(error));
      setBusy(false);
  ...
  const handleResetShootout = async () => {
    if (!currentFixture) return;
  ...
  const handleOpenShootout = async () => {
    if (!currentFixture) return;
    if (
      currentFixture.stage !== "knockout" ||
      currentFixture.status !== "in_progress" ||
      currentFixture.homeScore !== currentFixture.awayScore ||
      !["second_half", "extra_time", "penalties"].includes(currentFixture.period)
    ) {
      setToast("Shootouts are available only for tied knockout matches after normal or extra time.");
      return;
    }
    if (currentFixture.period !== "penalties") {
      setBusy(true);
      try {
        await postAppAction({ action: "startShootout", fixtureId: currentFixture.id });
        await refreshData();
      } catch (error) {
        setToast(errorMessage(error));
        setBusy(false);
  ...
  const handleSubmitGoal = async (e: React.FormEvent) => {
    ...
    if (!scorer) {
      setToast("Select the player who scored.");
      setBusy(false);
      return;
    }
    ...
    persistCurrentWAL({
      ...
      homeScore: goalType === "own_goal"
        ? (isHome ? currentFixture.homeScore : currentFixture.homeScore + 1)
        : (isHome ? currentFixture.homeScore + 1 : currentFixture.homeScore),
      awayScore: goalType === "own_goal"
        ? (isHome ? currentFixture.awayScore + 1 : currentFixture.awayScore)
        : (!isHome ? currentFixture.awayScore + 1 : currentFixture.awayScore),
    });
  ...
  const handleSubmitCard = async (e: React.FormEvent) => {
    ...
    if (!player) {
      setToast("Select the player receiving the card.");
      setBusy(false);
      return;
    }
  ...
  const handleSubmitSub = async (e: React.FormEvent) => {
    ...
    if (!playerOut || !playerIn || playerOut.id === playerIn.id) {
      setToast("Select two different players for the substitution.");
      setBusy(false);
      return;
    }
    ...
    try {
      await postAppAction({
        ...
        relatedPlayerName: playerIn.name,
  ...
  const handleDeleteEvent = async (eventId: string) => {
    ...
    const event = matchEvents.find((item) => item.id === eventId);
    const updatedEvents = matchEvents.filter((item) => item.id !== eventId);
    const scoreOverrides: Partial<MatchWAL> = { events: updatedEvents };
    if (event?.type === "goal" || event?.type === "penalty_goal") {
      if (event.entryId === currentFixture?.homeEntryId) scoreOverrides.homeScore = Math.max(0, (currentFixture?.homeScore || 0) - 1);
      if (event.entryId === currentFixture?.awayEntryId) scoreOverrides.awayScore = Math.max(0, (currentFixture?.awayScore || 0) - 1);
    } else if (event?.type === "own_goal") {
      if (event.entryId === currentFixture?.homeEntryId) scoreOverrides.awayScore = Math.max(0, (currentFixture?.awayScore || 0) - 1);
      if (event.entryId === currentFixture?.awayEntryId) scoreOverrides.homeScore = Math.max(0, (currentFixture?.homeScore || 0) - 1);
    }
    persistCurrentWAL(scoreOverrides);

    try {
      await postAppAction({ action: "deleteMatchEvent", eventId });
    } catch (error) {
      setToast(errorMessage(error));
      setBusy(false);
      return;
  ...
  const handleSubmitReport = async (e: React.FormEvent) => {
    ...
    try {
      await postAppAction({
        action: "completeMatchReport",
        fixtureId: currentFixture.id,
        potmPlayerId: potm?.id || null,
    ...
    return (
      <div className="min-h-screen bg-background text-foreground flex flex-col">
        ...
        <main className="flex-1 max-w-3xl w-full mx-auto p-6 flex items-center justify-center">
          <div className="panel-card rounded-3xl border border-border bg-card p-8 text-center space-y-3">
            <Clock size={34} className="mx-auto text-amber-500" />
            <h1 className="text-xl font-black">No referee assignments yet</h1>
            <p className="text-sm text-muted-foreground">Ask the tournament organizer to assign your account email to a fixture. It will appear here immediately after refresh.</p>
          </div>
        </main>
        <AppFooter />
      </div>
    ...
    >
      ...
      <main className="flex-1 max-w-3xl w-full mx-auto p-4 space-y-4">
          ...
          <div className="flex-1 min-w-[200px] flex items-center gap-2">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                const nextFixture = data.fixtures.find((fixture) => fixture.id === nextId);
                setSelectedFixtureId(nextId);
                setGoalTeam(nextFixture?.homeEntryId || "");
                setCardTeam(nextFixture?.homeEntryId || "");
                setSubTeam(nextFixture?.homeEntryId || "");
                setGoalScorerId("");
                setGoalAssistId("");
                setCardPlayerId("");
                setSubOutId("");
                setSubInId("");
                setShootoutKicks(localShootoutKicks(data.shootoutKicks, nextFixture));
                setPotmId(nextFixture?.potmPlayerId || "");
                setRefereeNotes(nextFixture?.refereeNotes || "");
                setWalRestoredNotice("");
              }}
        ...
        >
          ...
          >
            <button
              onClick={() => currentFixture?.status === "scheduled"
                ? handleUpdateClock(0, "first_half", "in_progress")
                : handleUpdateClock(displayMinute + 1, currentFixture?.period || "first_half")}
              disabled={busy || currentFixture?.status === "completed"}
              className={`p-3 rounded-2xl font-mono font-black text-sm transition shadow-sm disabled:opacity-40 disabled:cursor-not-allowed ${
                daylightMode
                  ? "bg-white text-black hover:bg-neutral-100 border-2 border-black"
                  : "bg-slate-800 hover:bg-slate-700 text-white"
              }`}
            >
              {currentFixture?.status === "scheduled" ? "Kick-Off" : "+1' Min"}
            </button>
            <button
              onClick={() => handleUpdateClock(Math.max(45, displayMinute), "half_time", "in_progress")}
              disabled={busy || currentFixture?.period !== "first_half"}
              className={`p-3 rounded-2xl font-black text-xs transition shadow-sm disabled:opacity-40 disabled:cursor-not-allowed ${
                daylightMode
            ...
            <button
              ...
              disabled={busy || currentFixture?.period !== "half_time"}
              className={`p-3 rounded-2xl font-black text-xs transition shadow-sm disabled:opacity-40 disabled:cursor-not-allowed ${
                daylightMode
            ...
            <button
              ...
              disabled={busy || currentFixture?.status !== "in_progress"}
              className={`p-3 rounded-2xl font-black text-xs transition shadow-sm disabled:opacity-40 disabled:cursor-not-allowed ${
                daylightMode
        ...
        >
          ...
          <div className="space-y-2">
            ...
            <span className={`text-[11px] font-semibold ${daylightMode ? "text-neutral-700" : "text-slate-400"}`}>
              Score follows verified goal events
            </span>
          </div>
          ...
          <div className="space-y-2">
            ...
            <span className={`text-[11px] font-semibold ${daylightMode ? "text-neutral-700" : "text-slate-400"}`}>
              Use Log Goal to change the score
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <button
            ...
            disabled={!canRecordEvents || busy}
            className={`p-4 rounded-2xl font-black text-sm flex flex-col items-center gap-1 shadow-lg transition disabled:opacity-40 disabled:cursor-not-allowed ${
              daylightMode
          ...
          <button
            onClick={() => void handleOpenShootout()}
            disabled={
              busy ||
              currentFixture?.stage !== "knockout" ||
              currentFixture?.status !== "in_progress" ||

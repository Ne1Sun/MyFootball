...
import type { AppData, ClubStaffAssignment, Division, Entry, Fixture, MatchEvent, Player, SchedulePolicy, SquadMember, Tournament } from "./components/types";
...
const tournamentLastDate = (tournament: Tournament) => {
  const [year, month, day] = tournament.startDate.split("-").map(Number);
  const value = new Date(Date.UTC(year, month - 1, day + Math.max(1, tournament.durationDays) - 1));
...
function EditTournament({
  ...
  const statusOptions: Record<string, Array<{ value: string; label: string }>> = {
    draft: [{ value: "draft", label: "Draft" }, { value: "registration_open", label: "Open registration" }],
    registration_open: [{ value: "registration_open", label: "Registration open" }, { value: "registration_closed", label: "Close registration" }],
    registration_closed: [{ value: "registration_closed", label: "Registration closed" }, { value: "registration_open", label: "Reopen registration" }],
    scheduled: [{ value: "scheduled", label: "Scheduled" }],
    live: [{ value: "live", label: "Live" }],
          ...
          <div className="form-grid">
            ...
            <label className="field">
              ...
              <select name="status" defaultValue={tournament.status}>
                {(statusOptions[tournament.status] || [{ value: tournament.status, label: tournament.status.replaceAll("_", " ") }]).map((option) => (
                  <option key={option.value} value={option.value}>{option.label}</option>
                ))}
              </select>
              <small>Scheduling, live and completed states advance automatically.</small>
            </label>
            <label className="field">
              <span>Venue / ground name</span>
...
function EditDivisionModal({
  ...
  const [busy, setBusy] = useState(false);
  const [format, setFormat] = useState(division.format);
  const [requirePlayers, setRequirePlayers] = useState(division.requirePlayers);
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setBusy(true);
    const form = new FormData(event.currentTarget);
    const saved = await onSaved({
      ...
      format,
      maxTeams: form.get("maxTeams"),
      groupsCount: form.get("groupsCount"),
      teamsAdvancingPerGroup: form.get("teamsAdvancingPerGroup"),
      maxSquadSize: form.get("maxSquadSize"),
      feeRupees: form.get("feeRupees"),
      winPoints: form.get("winPoints"),
      drawPoints: form.get("drawPoints"),
      lossPoints: form.get("lossPoints"),
      requirePlayers,
      requireDocuments: requirePlayers && form.get("requireDocuments") === "on",
      includeThirdPlace: form.get("includeThirdPlace") === "on",
    });
    setBusy(false);
    if (saved) onClose();
  ...
  return (
    <Modal title="Division rules" subtitle="These rules lock once the official schedule is generated." onClose={onClose}>
      <form onSubmit={submit}>
        <div className="modal-body">
          <div className="form-grid">
            <label className="field field-wide"><span>Division name</span><input name="name" defaultValue={division.name} required /></label>
            <label className="field"><span>Format</span><select value={format} onChange={(event) => setFormat(event.target.value)}><option value="group_knockout">Groups + knockout</option><option value="knockout">Knockout</option><option value="round_robin">Round robin league</option></select></label>
            <label className="field"><span>Maximum teams</span><input name="maxTeams" type="number" min="2" max="128" defaultValue={division.maxTeams} required /></label>
            {format === "group_knockout" && (<><label className="field"><span>Number of groups</span><input name="groupsCount" type="number" min="2" max="26" defaultValue={division.groupsCount} required /></label><label className="field"><span>Teams advancing per group</span><input name="teamsAdvancingPerGroup" type="number" min="1" max="32" defaultValue={division.teamsAdvancingPerGroup} required /></label></>)}
            <label className="field"><span>Maximum squad size</span><input name="maxSquadSize" type="number" min="1" max="50" defaultValue={division.maxSquadSize} required /></label>
            <label className="field"><span>Entry fee (₹ per team)</span><input name="feeRupees" type="number" min="0" defaultValue={division.feePaise / 100} required /></label>
            {format !== "knockout" && (<><label className="field"><span>Win points</span><input name="winPoints" type="number" min="0" max="10" defaultValue={division.winPoints} required /></label><label className="field"><span>Draw points</span><input name="drawPoints" type="number" min="0" max="10" defaultValue={division.drawPoints} required /></label><label className="field"><span>Loss points</span><input name="lossPoints" type="number" min="0" max="10" defaultValue={division.lossPoints} required /></label></>)}
            <label className="toggle-row field-wide"><div><strong>Player registration required</strong><small>Teams must maintain an eligible squad list</small></div><input type="checkbox" checked={requirePlayers} onChange={(event) => setRequirePlayers(event.target.checked)} /></label>
            {requirePlayers && <label className="toggle-row field-wide"><div><strong>Player documents required</strong><small>Require eligibility documents for registered players</small></div><input name="requireDocuments" type="checkbox" defaultChecked={division.requireDocuments} /></label>}
            {format !== "round_robin" && <label className="toggle-row field-wide"><div><strong>Third-place playoff</strong><small>Schedule the two semi-final losers</small></div><input name="includeThirdPlace" type="checkbox" defaultChecked={division.includeThirdPlace} /></label>}
          </div>
        </div>
        <div className="modal-actions"><button type="button" className="button secondary" onClick={onClose}>Cancel</button><button className="button primary" disabled={busy}>{busy ? <LoaderCircle className="spin" size={17} /> : <Pencil size={17} />} Save division rules</button></div>
      </form>
    </Modal>
  );
}
...
function GenerateFixturesModal({
  ...
  const selectedPolicy = policies.find((policy) => policy.divisionId === selectedDivisionId);
  ...
  const [startTime, setStartTime] = useState(selectedPolicy?.firstKickoffTime || "09:00");
  const [dayEndTime, setDayEndTime] = useState(selectedPolicy?.dayEndTime || "20:00");
  const [matchDuration, setMatchDuration] = useState(selectedPolicy?.matchDurationMinutes || 90);
  const [bufferMinutes, setBufferMinutes] = useState(selectedPolicy?.bufferMinutes || 15);
  const [minimumRest, setMinimumRest] = useState(selectedPolicy?.minimumRestMinutes || 60);
  const [pitches, setPitches] = useState(selectedPolicy?.pitchCount || 2);
  const [breakAfter, setBreakAfter] = useState(selectedPolicy?.longBreakAfterMatches || 0);
  const [longBreak, setLongBreak] = useState(selectedPolicy?.longBreakMinutes || 0);
  const [leagueLegs, setLeagueLegs] = useState(selectedPolicy?.leagueLegs || 1);
  const [weekendKnockouts, setWeekendKnockouts] = useState(selectedPolicy?.knockoutWeekendOnly || false);
  const [teamsAdvancing, setTeamsAdvancing] = useState(selectedDivision?.teamsAdvancingPerGroup || 2);
  const loadDivision = (divisionId: string) => {
    const nextDivision = divisions.find((division) => division.id === divisionId);
    const nextPolicy = policies.find((policy) => policy.divisionId === divisionId);
    setSelectedDivisionId(divisionId);
    setMode(nextDivision?.format || "group_knockout");
    setIncludeThirdPlace(nextDivision?.includeThirdPlace || false);
    setStartTime(nextPolicy?.firstKickoffTime || "09:00");
    setDayEndTime(nextPolicy?.dayEndTime || "20:00");
    setMatchDuration(nextPolicy?.matchDurationMinutes || 90);
    setBufferMinutes(nextPolicy?.bufferMinutes || 15);
    setMinimumRest(nextPolicy?.minimumRestMinutes || 60);
    setPitches(nextPolicy?.pitchCount || 2);
    setBreakAfter(nextPolicy?.longBreakAfterMatches || 0);
    setLongBreak(nextPolicy?.longBreakMinutes || 0);
    setLeagueLegs(nextPolicy?.leagueLegs || 1);
    setWeekendKnockouts(nextPolicy?.knockoutWeekendOnly || false);
    setTeamsAdvancing(nextDivision?.teamsAdvancingPerGroup || 2);
  };
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setBusy(true);
    ...
    const saved = await onSaved({
      ...
      longBreakAfterMatches: breakAfter,
      longBreakMinutes: longBreak,
      leagueLegs,
      knockoutWeekendOnly: weekendKnockouts,
  ...
  return (
    <Modal title="Generate official schedule" subtitle="Conflict checks protect pitches, rest time and tournament dates." onClose={onClose}>
      <form onSubmit={submit}>
        <div className="modal-body">
          <div className="form-grid">
            <label className="field field-wide">
              ...
              <select
                ...
                onChange={(event) => loadDivision(event.target.value)}
              >
                {divisions.map((item) => (
                  <option value={item.id} key={item.id}>
                    {item.name}
            ...
            {mode === "group_knockout" && (
              <label className="field">
                ...
                <input
                  ...
                  onChange={(event) => setTeamsAdvancing(Number(event.target.value))}
                  required
                />
              </label>
            )}
            <label className="field">
              <span>Start Date</span>
              <input name="startDate" type="date" min={tournament.startDate} max={tournamentLastDate(tournament)} defaultValue={tournament.startDate} required />
            </label>
            <label className="field">
            ...
            <label className="field">
              ...
              <input type="time" value={dayEndTime} onChange={(event) => setDayEndTime(event.target.value)} required />
            </label>
            <label className="field">
              <span>Available Pitches</span>
              <input type="number" min="1" max="12" value={pitches} onChange={(event) => setPitches(Number(event.target.value))} required />
            </label>
            <label className="field">
              <span>Match Duration (minutes)</span>
              <input type="number" min="20" max="180" value={matchDuration} onChange={(event) => setMatchDuration(Number(event.target.value))} required />
            </label>
            <label className="field">
              <span>Pitch Buffer (minutes)</span>
              <input type="number" min="0" max="120" value={bufferMinutes} onChange={(event) => setBufferMinutes(Number(event.target.value))} required />
            </label>
            <label className="field">
              <span>Minimum Team Rest (minutes)</span>
              <input type="number" min="0" max="1440" value={minimumRest} onChange={(event) => setMinimumRest(Number(event.target.value))} required />
            </label>
            {mode === "round_robin" && (
              <label className="field">
                <span>League Meetings</span>
                <select value={leagueLegs} onChange={(event) => setLeagueLegs(Number(event.target.value))}>
            ...
            <label className="field">
              <span>Long Break After Matches</span>
              <input type="number" min="0" max="20" value={breakAfter} onChange={(event) => setBreakAfter(Number(event.target.value))} />
              <small>Use 0 to disable scheduled long breaks.</small>
            </label>
            <label className="field">
              <span>Long Break Length (minutes)</span>
              <input type="number" min="0" max="240" value={longBreak} onChange={(event) => setLongBreak(Number(event.target.value))} disabled={breakAfter === 0} />
            </label>
            {mode !== "round_robin" && (
              <label className="toggle-row field-wide">
                <div>
            ...
            {mode !== "round_robin" && (
              <label className="toggle-row field-wide">
                <div>
                  <strong>Weekend knockout rounds</strong>
                  <small>Future knockout rounds begin on the next Saturday or Sunday</small>
                </div>
                <input type="checkbox" checked={weekendKnockouts} onChange={(event) => setWeekendKnockouts(event.target.checked)} />
              </label>
            )}
            <div className="smart-note field-wide">
              <Sparkles size={18} />
              <div>
                <strong>Production scheduling checks</strong>
                <p>No team or pitch can be double-booked. Every team receives the configured rest, and all fixtures must fit inside the tournament dates.</p>
              </div>
            </div>
          </div>
        </div>
...
function RescheduleFixtureModal({
  fixture,
  label,
  tournament,
  policy,
  ...
  const [busy, setBusy] = useState(false);
  const [date, time] = indiaDateTimeInput(fixture.kickoffAt).split("T");
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setBusy(true);
    const form = new FormData(event.currentTarget);
    const saved = await onSaved({
      action: "rescheduleFixture",
      fixtureId: fixture.id,
      date: form.get("date"),
      time: form.get("time"),
  ...
  const postpone = async () => {
    const reason = window.prompt("Why is this fixture being postponed?");
    if (!reason) return;
    setBusy(true);
    const saved = await onSaved({ action: "postponeFixture", fixtureId: fixture.id, reason });
    setBusy(false);
    if (saved) onClose();
  };
  return (
    <Modal title="Reschedule fixture" subtitle={`${label}. Conflicting pitches and insufficient team rest are blocked.`} onClose={onClose}>
      <form onSubmit={submit}>
        <div className="modal-body">
          <div className="form-grid">
            <label className="field">
              <span>New date</span>
              <input name="date" type="date" min={tournament.startDate} max={tournamentLastDate(tournament)} defaultValue={date} required />
            </label>
            <label className="field">
              <span>New kick-off</span>
              <input name="time" type="time" min={policy?.firstKickoffTime || "09:00"} max={policy?.dayEndTime || "20:00"} defaultValue={time} required />
            </label>
            <label className="field">
              <span>Pitch</span>
              <input name="pitch" type="number" min="1" max={policy?.pitchCount || 1} defaultValue={fixture.pitch} required />
            </label>
            <label className="field field-wide">
              <span>Change reason</span>
              <textarea name="reason" minLength={8} rows={3} placeholder="Weather delay, pitch change, travel disruption…" required />
            </label>
          </div>
        </div>
        <div className="modal-actions">
          {fixture.status === "scheduled" && (
            <button type="button" className="button secondary mr-auto" disabled={busy} onClick={() => void postpone()}>
              Postpone match
            </button>
          )}
          <button type="button" className="button secondary" onClick={onClose}>Cancel</button>
          <button className="button primary" disabled={busy}>
            {busy ? <LoaderCircle className="spin" size={17} /> : <CalendarDays size={17} />} Save new slot
          </button>
        </div>
      </form>
    </Modal>
...
export default function Dashboard({ user }: { user: ChatGPTUser }) {
  ...
  const [divisionTarget, setDivisionTarget] = useState<Division | null>(null);
  ...
  const tournamentHasRecords = (tournamentId: string) => {
    const divisionIds = new Set(data.divisions.filter((division) => division.tournamentId === tournamentId).map((division) => division.id));
    return data.entries.some((entry) => divisionIds.has(entry.divisionId)) || data.fixtures.some((fixture) => divisionIds.has(fixture.divisionId));
  };
  ...
  const handleSaveAction = async (payload: Record<string, unknown>) => {
    try {
      ...
      const successMessages: Record<string, string> = {
        ...
        updateDivision: "Division rules updated.",
        ...
        retryKnockoutProgression: "The next knockout round was scheduled successfully.",
        resetSchedule: "Unstarted schedule reset. Team placement is editable again.",
        rescheduleFixture: "Fixture rescheduled after conflict checks.",
        postponeFixture: "Fixture postponed and removed from the active pitch schedule.",
        cancelTournament: "Tournament cancelled while its records were preserved.",
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex">
                  ...
                  <div key={t.id} className="panel-card p-6 rounded-2xl bg-card border border-border space-y-4 shadow-sm">
                    ...
                    <div className="flex items-center justify-between pt-2">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          if (tournamentHasRecords(t.id)) {
                            const reason = window.prompt(`Why is "${t.name}" being cancelled? Its records will be preserved.`);
                            if (reason) void handleSaveAction({ action: "cancelTournament", tournamentId: t.id, reason });
                          } else if (confirm(`Permanently delete empty tournament "${t.name}"?`)) {
                            void handleSaveAction({ action: "deleteTournament", tournamentId: t.id });
                          }
                        }}
                        disabled={t.status === "cancelled" || t.status === "completed" || t.status === "live"}
                        className="p-2 text-muted-foreground hover:text-rose-500 transition disabled:cursor-not-allowed disabled:opacity-40"
                        aria-label={tournamentHasRecords(t.id) ? `Cancel ${t.name}` : `Delete ${t.name}`}
                        title={tournamentHasRecords(t.id) ? "Cancel tournament and preserve records" : "Delete empty tournament"}
                      >
                        <Trash2 size={16} />
                      </button>
                    </div>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  disabled={activeTournament?.status !== "registration_open"}
                  title={activeTournament?.status === "registration_open" ? "Register a team" : "Registration is closed"}
                  className="button primary inline-flex items-center gap-2 text-sm disabled:cursor-not-allowed disabled:opacity-50"
                >
                      ...
                      <tr key={entry.id} className="hover:bg-muted/30 transition">
                        ...
                        <td className="py-3 px-3">
                          <select
                            ...
                            disabled={busy || activeFixtures.some((fixture) => fixture.divisionId === entry.divisionId)}
                        ...
                        <td className="py-3 px-3">
                          <input
                            type="number"
                            min="1"
                            max={activeDivisions.find((division) => division.id === entry.divisionId)?.maxTeams || 128}
                            defaultValue={entry.seed ?? ""}
                            disabled={busy || activeFixtures.some((fixture) => fixture.divisionId === entry.divisionId)}
                            onBlur={(event) => {
                              const next = event.currentTarget.value;
                              if (next === String(entry.seed ?? "")) return;
                            ...
                            className="w-16 rounded-lg border border-border bg-muted p-1.5 text-xs text-foreground"
                            aria-label={`Seed for ${entry.teamName}`}
                            placeholder="—"
                          />
                        </td>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Configure division rules, publish conflict-free schedules and manage every match through completion.</p>
                </div>
                <button
                  onClick={() => setFixtureOpen(true)}
                  disabled={activeDivisions.every((division) => activeFixtures.some((fixture) => fixture.divisionId === division.id))}
                  className="button primary inline-flex items-center gap-2 text-sm disabled:cursor-not-allowed disabled:opacity-50"
                >
              ...
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                {activeDivisions.map((division) => {
                  const hasSchedule = activeFixtures.some((fixture) => fixture.divisionId === division.id);
                  return (
                    <div key={division.id} className="panel-card rounded-2xl border border-border bg-card p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <strong className="text-sm text-foreground">{division.name}</strong>
                          <p className="mt-1 text-xs text-muted-foreground">{division.format.replaceAll("_", " ")} · Up to {division.maxTeams} teams · {division.winPoints}/{division.drawPoints}/{division.lossPoints} points</p>
                        </div>
                        <button
                          type="button"
                          disabled={hasSchedule}
                          onClick={() => setDivisionTarget(division)}
                          className="button secondary shrink-0 px-3 py-1.5 text-xs disabled:cursor-not-allowed disabled:opacity-50"
                          title={hasSchedule ? "Reset the unstarted schedule before editing rules" : "Edit division rules"}
                        >
                          <Pencil size={13} /> Rules
                        </button>
                      </div>
                    </div>
                  );
              ...
              {activeTournament?.status === "registration_open" && (
                <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-800 dark:text-amber-200">
                  Close registration from <strong>Edit tournament</strong> after resolving pending entries, then generate the official schedule.
                </div>
              )}

              {activeDivisions.some((division) => activeFixtures.some((fixture) => fixture.divisionId === division.id)) && (
                <div className="panel-card rounded-2xl border border-border bg-card p-5">
                  <h3 className="font-bold text-foreground">Published division schedules</h3>
                  <p className="mt-1 text-xs text-muted-foreground">A schedule can only be reset before any match begins. Referee assignments for that division are removed with it.</p>
                  <div className="mt-3 divide-y divide-border">
                    {activeDivisions.map((division) => {
                      const divisionFixtures = activeFixtures.filter((fixture) => fixture.divisionId === division.id);
                      if (!divisionFixtures.length) return null;
                      const resetAllowed = activeTournament.status === "scheduled" && divisionFixtures.every((fixture) => ["scheduled", "postponed"].includes(fixture.status));
                      return (
                        <div key={division.id} className="flex flex-wrap items-center justify-between gap-3 py-3">
                          <div>
                            <strong className="text-sm text-foreground">{division.name}</strong>
                            <p className="text-xs text-muted-foreground">{divisionFixtures.length} fixtures · {resetAllowed ? "Unstarted" : "Competition underway"}</p>
                          </div>
                          <button
                            type="button"
                            disabled={!resetAllowed || busy}
                            className="button secondary text-xs disabled:cursor-not-allowed disabled:opacity-50"
                            onClick={() => {
                              if (window.confirm(`Reset all unstarted fixtures for ${division.name}?`)) {
                                void handleSaveAction({ action: "resetSchedule", divisionId: division.id });
                              }
                            }}
                          >
                      ...
                      return (
                        <div key={fixture.id} className="py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">{fixture.roundName} · {timeLabel(fixture.kickoffAt)} · Pitch {fixture.pitch} · {fixture.status.replaceAll("_", " ")} · {official ? `Referee: ${official.userEmail}` : "Referee unassigned"}</p>
                          </div>
                          <div className="flex flex-wrap gap-2">
                            ...
                            <button
                              className="button secondary text-xs px-3 py-1.5 disabled:cursor-not-allowed disabled:opacity-50"
                              disabled={fixture.status === "postponed"}
                              onClick={() => handleOpenMatchday(fixture.id)}
                              title={fixture.status === "postponed" ? "Set a new time before opening matchday" : "Open matchday controls"}
                            >
                              <Swords size={14} /> Matchday
                            </button>
                            {["scheduled", "postponed"].includes(fixture.status) && (
                              <button className="button secondary text-xs px-3 py-1.5" onClick={() => setRescheduleTarget(fixture)}>
                                <CalendarDays size={14} /> {fixture.status === "postponed" ? "Set new time" : "Reschedule"}
                              </button>
                            )}
      ...
      {fixtureOpen && activeTournament && (
        <GenerateFixturesModal
          divisions={activeDivisions.filter((division) => !activeFixtures.some((fixture) => fixture.divisionId === division.id))}
          tournament={activeTournament}
          policies={data.schedulePolicies}
      ...
      {rescheduleTarget && activeTournament && (
        <RescheduleFixtureModal
          fixture={rescheduleTarget}
          label={`${activeEntries.find((entry) => entry.id === rescheduleTarget.homeEntryId)?.teamName || "Home"} vs ${activeEntries.find((entry) => entry.id === rescheduleTarget.awayEntryId)?.teamName || "Away"}`}
          tournament={activeTournament}
          policy={data.schedulePolicies.find((policy) => policy.divisionId === rescheduleTarget.divisionId)}
      ...
      {divisionTarget && (
        <EditDivisionModal
          ...
          onClose={() => setDivisionTarget(null)}
          onSaved={handleSaveAction}
        />
      )}
    </div>

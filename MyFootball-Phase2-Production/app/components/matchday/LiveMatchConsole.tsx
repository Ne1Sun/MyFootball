...
export function LiveMatchConsole({
  ...
  const [selectedFixtureId, setSelectedFixtureId] = useState<string>(
    initialFixtureId || fixtures.find((f) => f.status === "in_progress")?.id || fixtures.find((f) => f.status === "scheduled")?.id || fixtures[0]?.id || ""
  );
  ...
  const handleUpdateClock = async (minute: number, period: string, status?: string) => {
    if (!currentFixture) return;
    setBusy(true);
    const saved = await onSaveAction({
  ...
  const handleStartMatch = async () => {
    if (!currentFixture) return;
    await handleUpdateClock(currentFixture.matchClockMinute || 0, "first_half", "in_progress");
  };

  const handlePauseMatch = async () => {
    if (!currentFixture) return;
    await handleUpdateClock(Math.max(45, displayMinute), "half_time", "in_progress");
  };

  const handleStartSecondHalf = async () => {
    if (!currentFixture) return;
    await handleUpdateClock(Math.max(45, displayMinute), "second_half", "in_progress");
  };
  ...
  const handleSubmitGoal = async (e: React.FormEvent) => {
    ...
    if (!scorer || (assist && assist.id === scorer.id)) {
      setBusy(false);
      return;
    }
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3 min-w-max">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              const nextFixture = fixtures.find((fixture) => fixture.id === nextId);
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-slate-800 to-slate-950 text-white p-6 sm:p-8 shadow-2xl border border-slate-700/50">
        ...
        <div className="relative z-10 flex flex-wrap items-center justify-between gap-4 pb-6 border-b border-slate-700/50">
          ...
          <div className="flex items-center gap-2">
            {currentFixture?.status === "scheduled" ? (
              ...
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-700 text-slate-300 font-bold text-xs">
                <CheckCircle2 size={13} /> Result locked
              </span>
            ) : (
              <>
                <button
                  onClick={() => handleUpdateClock(displayMinute + 1, currentFixture?.period || "first_half")}
                  disabled={busy}
                ...
                {currentFixture?.period === "first_half" && (
                  <button
                ...
                {currentFixture?.period === "half_time" && (
                  <button
                    ...
                    disabled={busy}
                    className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-xs font-bold text-white transition"
                  >
                    <Play size={13} /> Start 2nd Half
                  </button>
                )}
                <button
        ...
        <div className="relative z-10 pt-6 border-t border-slate-700/50 flex flex-wrap items-center justify-center gap-3">
          <button
            onClick={() => {
              setGoalMinute(displayMinute || 1);
              setActiveModal("goal");
            }}
            disabled={!canRecordEvents || busy}
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm shadow-md transition disabled:opacity-40 disabled:cursor-not-allowed"
          >
            <span>⚽</span> Record Goal / Scorer
          </button>
          <button
            onClick={() => {
              setCardMinute(displayMinute || 1);
              setActiveModal("card");
            }}
            disabled={!canRecordEvents || busy}
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-amber-600 hover:bg-amber-500 text-white font-bold text-sm shadow-md transition disabled:opacity-40 disabled:cursor-not-allowed"
          >
            <span>🟨</span> Issue Booking / Card
          </button>
          <button
            onClick={() => {
              setSubMinute(displayMinute || 45);
              setActiveModal("sub");
            }}
            disabled={!canRecordEvents || busy}
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm shadow-md transition disabled:opacity-40 disabled:cursor-not-allowed"
          >
            <span>🔄</span> Log Substitution
          </button>
          <button
            onClick={() => setActiveModal("potm")}
            disabled={currentFixture?.status === "completed" || busy}
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-700 hover:bg-slate-600 text-amber-400 font-bold text-sm border border-slate-600 transition disabled:opacity-40 disabled:cursor-not-allowed"
          >

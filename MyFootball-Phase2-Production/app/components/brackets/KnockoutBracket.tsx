...
export function KnockoutBracket({
  ...
  const canRetryProgression = useMemo(() => {
    ...
    const progressionFixtures = knockoutFixtures.filter((fixture) =>
      fixture.bracketRound && progressionRounds.has(fixture.bracketRound),
    );
    if (!progressionFixtures.length || knockoutFixtures.some((fixture) => fixture.bracketRound === "final")) return false;
    const latestRoundNumber = Math.max(...progressionFixtures.map((fixture) => fixture.roundNumber));
    const latestRound = progressionFixtures.filter((fixture) => fixture.roundNumber === latestRoundNumber);
    return latestRound.length > 0 && latestRound.every((fixture) => fixture.status === "completed");
  }, [knockoutFixtures]);
  ...
  const renderMatchCard = (fixture: Fixture, title: string, isChampionStage = false) => {
    ...
    const isGrandFinal = fixture.bracketRound === "final";
      ...
      >
        ...
        <div className="mt-3 pt-2.5 border-t border-border/60 flex items-center justify-between gap-2">
          ...
          <span className="text-[11px] text-muted-foreground text-right">
            {fixture.winnerEntryId
              ? `${entries.find((entry) => entry.id === fixture.winnerEntryId)?.teamName || "Winner"} advanced automatically`
              : "Winner advances after the verified final whistle"}
          </span>
        </div>
      </div>
    );
          ...
          {canRetryProgression && (
            <button
              ...
              onClick={async () => {
                setBusy(true);
                try {
                  await onSaveAction({ action: "retryKnockoutProgression", divisionId: division.id });
                } finally {
                  setBusy(false);
                }
              }}
            ...
            >
              {busy ? <LoaderCircle className="spin" size={14} /> : <FastForward size={14} />} Create next round
            </button>
          )}
        </div>
      </div>

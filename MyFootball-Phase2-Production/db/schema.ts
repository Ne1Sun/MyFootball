...
export const divisions = sqliteTable("divisions", {
  ...
  includeThirdPlace: integer("include_third_place", { mode: "boolean" }).notNull().default(false),
  knockoutByeEntryIds: text("knockout_bye_entry_ids").notNull().default("[]"),
...
export const schedulePolicies = sqliteTable("schedule_policies", {
  ...
  firstKickoffTime: text("first_kickoff_time").notNull().default("09:00"),
  ...
  minimumRestMinutes: integer("minimum_rest_minutes").notNull().default(60),
  pitchCount: integer("pitch_count").notNull().default(2),
  longBreakAfterMatches: integer("long_break_after_matches").notNull().default(0),
  longBreakMinutes: integer("long_break_minutes").notNull().default(0),
  leagueLegs: integer("league_legs").notNull().default(1),
  knockoutWeekendOnly: integer("knockout_weekend_only", { mode: "boolean" }).notNull().default(false),
  updatedAt: text("updated_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [
  uniqueIndex("schedule_policies_division_uq").on(table.divisionId),
]);
...
export const clubStaffAssignments = sqliteTable("club_staff_assignments", {
  id: text("id").primaryKey(),
  ...
  uniqueIndex("club_staff_club_user_role_uq").on(table.clubId, table.userEmail, table.role),
  index("club_staff_user_status_idx").on(table.userEmail, table.status),
]);
...
export const fixtures = sqliteTable("fixtures", {
  ...
  refereeNotes: text("referee_notes").notNull().default(""),
  winnerEntryId: text("winner_entry_id"),
  loserEntryId: text("loser_entry_id"),
  publishedAt: text("published_at").notNull().default(sql`CURRENT_TIMESTAMP`),
  createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`),
}, (table) => [index("fixtures_division_kickoff_idx").on(table.divisionId, table.kickoffAt)]);

export const fixtureOfficials = sqliteTable("fixture_officials", {
  id: text("id").primaryKey(),
  ...
  uniqueIndex("fixture_official_fixture_role_uq").on(table.fixtureId, table.role),
  index("fixture_official_user_status_idx").on(table.userEmail, table.status),
]);
...
export const shootoutKicks = sqliteTable("shootout_kicks", {
  ...
  uniqueIndex("shootout_fixture_sequence_uq").on(table.fixtureId, table.sequence),
  index("shootout_fixture_idx").on(table.fixtureId, table.createdAt),
]);
...
export const auditLogs = sqliteTable("audit_logs", {
  ...
  index("audit_tournament_created_idx").on(table.tournamentId, table.createdAt),

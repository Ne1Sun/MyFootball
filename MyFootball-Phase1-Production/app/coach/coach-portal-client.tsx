...
import type { Announcement, Division, Entry, Fixture, MatchEvent, Player, SquadMember, Tournament } from "../components/types";
...
export function CoachPortalClient({ initialData }: { initialData: CoachPortalData }) {
  ...
  const [activeTab, setActiveTab] = useState<"roster" | "squads" | "fixtures" | "tournaments" | "updates">("roster");
  ...
  const refreshData = async () => {
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(body.error || "Could not refresh team data");
      }
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Could not refresh team data");
    }
  };

  const saveSquad = async (squad: unknown[]) => {
    if (!activeEntry) return;
    const response = await fetch("/api/app", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "updateSquad", entryId: activeEntry.id, squad }),
    });
    const body = await response.json();
    if (!response.ok) throw new Error(body.error || "Could not update the squad");
    await refreshData();
  };
  ...
  const handleAddPlayer = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch("/api/app", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "addPlayer", clubId: activeClub.id, name, jerseyNumber, position, isCaptain, dateOfBirth: dateOfBirth || null }),
      });
      const body = await res.json();
      ...
      setError(reason instanceof Error ? reason.message : "Could not add player");
    } finally {
      setBusy(false);
    }
  };
  ...
  const handleToggleStarting = async (playerId: string, currentStarting: boolean) => {
    ...
    try { await saveSquad(updatedSquad); } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not update squad"); } finally { setBusy(false); }
  };
  ...
  const handleSwapPlayers = async (pitchId: string, benchId: string) => {
    if (!activeEntry) return;
    setBusy(true);
    const updatedSquad = entrySquad.map((member) =>
      member.playerId === pitchId
        ? { ...member, isStarting: false }
        : member.playerId === benchId
          ? { ...member, isStarting: true }
          : member,
    );
    try { await saveSquad(updatedSquad); } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not swap players"); } finally { setBusy(false); }
  };
  ...
  if (!data.clubs.length) {
    return (
      <div className="min-h-screen bg-background text-foreground flex flex-col">
        ...
        <main className="flex-1 max-w-3xl w-full mx-auto p-4 lg:p-8 flex items-center">
          <div className="w-full panel-card rounded-3xl border border-border bg-card p-8 sm:p-12 text-center shadow-xl">
            <ShieldCheck size={44} className="mx-auto text-primary" />
            <h1 className="mt-4 text-2xl font-black">Create or register a team first</h1>
            <p className="mx-auto mt-2 max-w-xl text-sm leading-relaxed text-muted-foreground">
              Your coach hub activates automatically when you register a team in a tournament. You can also create your own tournament from the organizer hub using this same account.
            </p>
            <div className="mt-6 flex flex-wrap justify-center gap-3">
              <a href="/discover" className="button primary inline-flex items-center gap-2">
                <Trophy size={16} /> Find a tournament
              </a>
              <a href="/organize" className="button secondary inline-flex items-center gap-2">
                <Plus size={16} /> Create a tournament
              </a>
            </div>
          </div>
        </main>
    ...
    <div className="min-h-screen bg-background text-foreground flex flex-col font-sans selection:bg-amber-500 selection:text-slate-950">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 lg:p-8 space-y-6">
              ...
              <div className="bg-slate-800/80 p-3 rounded-2xl border border-slate-700 space-y-1">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const teamIds = new Set(data.teams.filter((team) => team.clubId === clubId).map((team) => team.id));
                    setSelectedClubId(clubId);
                    setSelectedEntryId(data.entries.find((entry) => teamIds.has(entry.teamId))?.id || "");
                  }}
        ...
        <div className="cascade-2 flex flex-wrap items-center gap-2 border-b border-border pb-3">
          ...
          >
            <Bell size={15} /> Organizer Updates ({data.announcements.length})
          </button>
        </div>
          ...
          <div className="cascade-3 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {clubPlayers.map((player) => (
                <div
                  key={player.id}
                  className="panel-card p-4 rounded-2xl bg-card border border-border space-y-3 shadow-sm"
                >
                  <div className="flex items-center justify-between">
        ...
        {activeTab === "squads" && (
          <div className="cascade-3 space-y-6">
            {clubEntries.length === 0 ? (
              <div className="panel-card rounded-3xl border border-border bg-card p-10 text-center">
                <Shield size={36} className="mx-auto text-muted-foreground" />
                <h3 className="mt-3 font-extrabold">No active tournament entry</h3>
                <p className="mx-auto mt-1 max-w-md text-xs text-muted-foreground">
                  Register one of this club&apos;s teams in a tournament before building a matchday squad.
                </p>
                <a href="/discover" className="button primary mt-5 inline-flex items-center gap-2 text-xs">
            ...
            <div className="panel-card p-6 rounded-3xl bg-card border border-border shadow-md">
              <TacticalPitch
                ...
                onSwapPlayer={(pitchId, benchId) => void handleSwapPlayers(pitchId, benchId)}
        ...
        {activeTab === "fixtures" && (
          <div className="cascade-3 space-y-4">
            ...
            {clubFixtures.length === 0 ? (
              <div className="panel-card rounded-3xl border border-border bg-card p-10 text-center text-sm text-muted-foreground">
                <Calendar size={34} className="mx-auto mb-3" />
                No fixtures have been published for this club yet.
              </div>
            ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {clubFixtures.map((f) => {
                ...
                const fixtureDivision = data.divisions.find((division) => division.id === f.divisionId);
                const tournamentId = fixtureDivision?.tournamentId;
                return (
                  <button
                    key={f.id}
                    type="button"
                    disabled={!tournamentId}
                    onClick={() => tournamentId && (window.location.href = `/tournament/${tournamentId}`)}
                    className="interactive-card panel-card p-4 rounded-2xl bg-card border border-border space-y-2 text-left shadow-sm hover:border-primary/40 disabled:cursor-not-allowed disabled:opacity-60"
                    title={tournamentId ? "Open tournament match centre" : "Tournament unavailable"}
                  >
                    ...
                    <span className="block text-right text-[10px] font-bold text-primary">Open match centre →</span>
                  </button>
                );
              })}
            </div>
        ...
        {/* TAB 5: ORGANIZER UPDATES */}
        {activeTab === "updates" && (
          <div className="cascade-3 space-y-4">
            <div>
              <h3 className="font-extrabold text-lg text-foreground">Organizer Updates</h3>
              <p className="text-xs text-muted-foreground">
                Participant notices and private coach instructions for your registered tournaments.
              </p>
            </div>
            {data.announcements.length === 0 ? (
              <div className="panel-card rounded-3xl border border-border bg-card p-10 text-center text-sm text-muted-foreground">
                <Bell size={34} className="mx-auto mb-3" />
                No organizer updates have been published for your teams.
              </div>
            ) : (
              <div className="space-y-3">
                {data.announcements.map((announcement) => {
                  const tournament = data.tournaments.find((item) => item.id === announcement.tournamentId);
                  return (
                    <article key={announcement.id} className="panel-card rounded-2xl border border-border bg-card p-5 shadow-sm">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <strong className="text-sm text-foreground">{tournament?.name || "Tournament update"}</strong>
                        <span className="rounded-full bg-primary/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-primary">
                          {announcement.audience === "coaches_only" ? "Coaches only" : "All participants"}
                        </span>
                      </div>
                      <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-foreground">{announcement.body}</p>
                      <time className="mt-3 block text-[11px] text-muted-foreground" dateTime={announcement.createdAt}>
                        {new Date(announcement.createdAt).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })}
                      </time>
                    </article>
                  );
                })}
